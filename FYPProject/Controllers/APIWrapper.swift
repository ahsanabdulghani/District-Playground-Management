...
class APIWrapper{
    static let imgBaseURLString = "http://10.211.55.4/DPMSapi/"
    private let baseURLString = "http://10.211.55.4/DPMSapi/api/"
    
    static var userData = Login()
    ...
    func getMethodCall(controllerName:String,actionName:String)->APIMessage {
        ...
        let task = session.dataTask(with: urlRequest){
            ...
            print("**", apiMessage.ResponseMessage)
            group.leave()
        }
        task.resume()
        group.wait()
